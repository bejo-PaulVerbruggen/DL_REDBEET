...
import numpy as np
#define label source
label_src = '/mnt/share/temp/DCDL_labelfiles/REDBEET/detailed_labels_side_germ.xlsx'
...
for _,row in metadata.iterrows():
    ...
    score = row['score']
    multigerm = row['multigerm_prediction']
    if row['imagefile'] != 'file not present':
        ...
        cv2.putText(image,f'multigerm: {str(multigerm)}',(10,60),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),1,cv2.LINE_AA)
        cv2.putText(image,f'score: {str(score)}',(10,75),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),1,cv2.LINE_AA)
