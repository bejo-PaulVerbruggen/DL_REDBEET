...
model_germ = '/mnt/share/temp/DCDL_labelfiles/Models/BEET_MULTIGERM/BEET_MULTIGERM_9.onnx'
#Load model
session = onnxruntime.InferenceSession(model_src, None)
session_germ = onnxruntime.InferenceSession(model_germ, None)
print('models loaded!')
# get the name of the first input of the model
input_name = session.get_inputs()[0].name
input_name_germ = session_germ.get_inputs()[0].name 
...
for file in alllabels['imagefile']:
    if file != 'file not present':
        ...
        if res[0] < 0.7:
            scores.append('top')
            multi_germ.append('wrong side')
        else:
            i += 1
            scores.append('side')
            germ = session_germ.run([], {input_name_germ: input_data})
            res_germ = postprocess(germ)
            if res_germ [0] < 0.5:
                multi_germ.append('no')
            else:
                multi_germ.append('yes')
        ...
        multi_germ.append('NA')
        scores.append('None')

alllabels['side_prediction'] = scores
alllabels['multigerm_prediction'] = multi_germ
#Write to Excel
label_src = '/mnt/share/temp/DCDL_labelfiles/REDBEET'
alllabels.to_excel(os.path.join(label_src,'detailed_labels_side_germ.xlsx'),index=False)
